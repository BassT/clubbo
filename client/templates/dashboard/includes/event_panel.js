...
Template.eventPanel.onCreated(function () {
    'use strict';
    this.subscribe('responsesByUserIdAndEventId', Meteor.userId(), Template.currentData().event._id);
});
...
Template.eventPanel.helpers({
    ...
    responseMissing: function (event) {
        ...
        if (Meteor.user().profile.teamIds.indexOf(event.teamId) !== -1 && event.responseRequired) {
            eventResponses = Responses.find({eventId: event._id, userId: Meteor.userId()});
    ...
    responseButtonStyle: function (event) {
        'use strict';
        var response;
        response = Responses.findOne({eventId: event._id, userId: Meteor.userId()});
...
Template.eventPanel.events({
    ...
    'click .instant-response': function (e) {
        ...
        Meteor.call('insertResponse', Meteor.userId(), event._id, value, '', function (error) {
            if (error) {
                bootbox.alert({
