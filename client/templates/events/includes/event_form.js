...
Template.eventForm.helpers({
    ...
    departments: function () {
        'use strict';
        return Departments.find({_id: {$in: Meteor.user().profile.departmentIds}}).fetch();
    },
    teams: function () {
        'use strict';
...
Template.eventForm.events({
    'change select#club, change select#department, change select#team': function (changeEvent) {
        'use strict';
        var $changedSelect, contextType, contextId, possibleContextIds, i, $contextDiv, $departmentSelect,
            $teamSelect, implicitContexts, implicitContext;
        ...
        possibleContextIds = EventsHelper.getPossibleContextIds(contextType, contextId);
        if (contextType !== 'team') {
            $departmentSelect = $contextDiv.find('select#department');
            $teamSelect = $contextDiv.find('select#team');

            if (contextType === 'club') {
                if (!(possibleContextIds.departmentIds && _.contains(possibleContextIds.departmentIds, $departmentSelect.val()))) {
                    $departmentSelect.val('null');
                }
            }
            if (!(possibleContextIds.teamIds && _.contains(possibleContextIds.teamIds, $teamSelect.val()))) {
                $teamSelect.val('null');
            }
        }

        // Set implicit contexts
        implicitContexts = EventsHelper.getImplicitContextIds(contextType, contextId);
    ...
    'change #visibility #clubs input[type=checkbox]': function (changeEvent, template) {
        'use strict';
        ...
        _.each(club.departmentIds, function (departmentId) {
            $departmentsDiv.find('input[data-context-id="' + departmentId + '"]').prop('checked', checked);
        });

        $teamsDiv = template.$('#visibility #teams');
        _.each(club.teamIds, function (teamId) {
            $teamsDiv.find('input[data-context-id="' + teamId + '"]').prop('checked', checked);
        });
    },
    'change #visibility #departments input[type=checkbox]': function (changeEvent, template) {
        'use strict';
