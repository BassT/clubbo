...
function getRelevantYeses(position) {
    'use strict';
    var users = Meteor.users.find({'profile.positions': {$in: [position]}}).fetch(),
        i, user, result, response;

    result = [];

    for (i = 0; i < users.length; i = i + 1) {
        user = users[i];
        response = Responses.findOne({userId: user._id, eventId: Template.parentData().event._id, value: 'yes'});
        if (response) {
...
Template.eventDetails.helpers({
    ...
    getMessageOrNoteOfAbsence: function () {
        ...
        absence = Absences.findOne(userAbsencesQuery(response.userId, moment(event.start).toDate(), moment(event.end).toDate()), {
            sort: ['start']
        });
    ...
    messageAvailable: function () {
        ...
        absencesCursor = Absences.find(userAbsencesQuery(response.userId, moment(event.start).toDate(), moment(event.end).toDate()));
    ...
    userHasAbsence: function () {
        ...
        absencesCursor = Absences.find(userAbsencesQuery(user._id, moment(event.start).toDate(), moment(event.end).toDate()));
    ...
    getAbsence: function () {
        ...
        user = this;
        event = Template.parentData().event;
        absence = Absences.findOne(userAbsencesQuery(user._id, moment(event.start).toDate(), moment(event.end).toDate()), {
            sort: ['start']
        });
        return moment(absence.start).format('DD.MM.YYYY') + ' - ' + moment(absence.end).format('DD.MM.YYYY') + ': ' + absence.note;
    },
    ...
    teamsOfRespondents: function () {
        ...
        return Teams.find({_id: {$in: Session.get('selectedTeamIds')}}).map(function (team) {
            return {
                _id: team._id,
                name: team.name,
                numberOfRespondents: Meteor.users.find({
