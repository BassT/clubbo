...
Template.teamSelectionParagraph.onCreated(function () {
    ...
    template.autorun(function() {
        template.subscribe('teamsByIds', template.possibleTeamIds.get(), function () {
...
Template.teamSelectionParagraph.helpers({
    selectedTeams: function () {
        'use strict';
        return Teams.find({_id: {$in: Session.get('selectedTeamIds')}}).map(function (team, index) {
            var separator = false;
            if (index > 0) {
                separator = true;
            }
    ...
    notAllTeamsSelected: function () {
        'use strict';
        return _.difference(Template.instance().possibleTeamIds.get(), Session.get('selectedTeamIds')).length > 0;
    },
    notSelectedTeams: function () {
        'use strict';
        return Teams.find({_id: {
            $in: _.difference(Template.instance().possibleTeamIds.get(), Session.get('selectedTeamIds'))
...
Template.teamSelectionParagraph.events({
    'click .team': function (event) {
        ...
        if ($link.hasClass('selected')) {
            Session.set('selectedTeamIds', _.without(selectedTeamIds, $link.data('teamId')));
