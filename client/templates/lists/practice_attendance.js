...
Template.practiceAttendance.onCreated(function() {
  ...
  template.autorun(function() {
    ...
    userIds = Meteor.users
      .find({ "profile.teamIds": { $in: teamIds } })
      .map(function(user) {
        return user._id;
      });
    ...
    start = moment()
      ...
      .format();
    tags = ["Training"];
    eventIds = Events.find({ tags: { $in: ["Training"] } }).map(function(
      event
    ) {
      return event._id;
    });
...
Template.practiceAttendance.helpers({
  ...
  users: function() {
    "use strict";
    return Meteor.users
      .find({ "profile.teamIds": { $in: Session.get("selectedTeamIds") } })
      .fetch();
  },
  practiceResponses: function() {
    ...
    return Responses.find({
      eventId: { $in: template.eventIds.get() },
      userId: { $in: template.userIds.get() }
    }).fetch();
  }
});
