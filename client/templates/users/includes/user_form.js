...
Template.userForm.onRendered(function () {
    ...
    if (this.data.user) {
        ...
        if (user.profile.birthday && user.profile.birthday !== '') {
            birthday.datepicker('update', moment(user.profile.birthday).format('DD.MM.YYYY'));
        }
        ...
        if (positions) {
            ...
            for (i = 0; i < positions.length; i = i + 1) {
                posCheckboxes.find('input[value=' + positions[i] + ']').prop('checked', true);
            }
        }
    }
});
...
Template.userForm.helpers({
    teams: function () {
        ...
        teams = Teams.find({_id: {
            $in: _.flatten([Meteor.user().profile.teamIds, Roles.getGroupsForUser(Meteor.user(), 'admin')])}
        }).fetch();
