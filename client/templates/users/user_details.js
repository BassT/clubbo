...
function userAbsencesQuery(userId) {
  ...
  return {
    $and: [
      { userId: userId },
      { $or: [{ start: { $gt: new Date() } }, { end: { $gt: new Date() } }] }
    ]
  };
}
...
Template.userDetails.helpers({
  ...
  userCanEdit: function() {
    ...
    return (
      _.intersection(editedUser.profile.teamIds, editingUserAdminships).length >
        0 || editingUser._id === editedUser._id
    );
  },
  ...
  hasAbsences: function() {
    ...
    return (
      Absences.find(
        userAbsencesQuery(Template.instance().user.get()._id)
      ).count() > 0
    );
  },
  ...
  teams: function() {
    "use strict";
    return Teams.find({
      _id: { $in: Template.instance().user.get().profile.teamIds }
    }).fetch();
  }
});
