...
Meteor.methods({
  ...
  updateProfile: function(user) {
    "use strict";
    var loggedInUser, oldUserProfile, oldTeamIds, newTeamIds, setObj;
    ...
    if (
      _.difference(oldTeamIds, user.profile.teamIds).length > 0 ||
    ...
    Teams.update(
      { _id: { $in: _.difference(oldTeamIds, newTeamIds) } },
      {
        $pull: { userIds: user._id }
      }
    );
    Teams.update(
      { _id: { $in: _.difference(newTeamIds, oldTeamIds) } },
      {
