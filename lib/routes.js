...
Router.onBeforeAction(function () {
    ...
    if (!Meteor.userId()) {
        ...
        this.wait(Meteor.subscribe('teamsByIds', Meteor.user().profile.teamIds, function () {
            var teamIds = Meteor.user().profile.teamIds;
            Teams.find({_id: {$in: Meteor.user().profile.teamIds}}).map(function (team) {
    ...
    if (this.ready()) {
        this.render('eventDetails', {
            data: function () {
                ...
                users = Meteor.users.find({'profile.teamIds': {$in: [event.teamId]}});
                responses = Responses.find({eventId: event._id, userId: {$in: users.map(function (user) {
                    return user._id;
                })}}).fetch();
                return {
                    event: event,
                    tags: event.tags,
                    responses: responses,
                    yeses: Responses.find({eventId: event._id, value: 'yes', userId: {$in: users.map(function (user) {
                        return user._id;
                    })}}).fetch(),
                    nos: Responses.find({eventId: event._id, value: 'no', userId: {$in: users.map(function (user) {
                        return user._id;
                    })}}).fetch(),
                    users: users
                };
...
Router.route('/events/:_id/respond', {
    ...
    subscriptions: function () {
        'use strict';
        return [
            Meteor.subscribe('responsesByUserIdAndEventId', Meteor.userId(), this.params._id),
    ...
    data: function () {
        ...
        return {
            ...
            response: Responses.findOne({eventId: this.params._id, userId: Meteor.userId()})
        };
    }
});
...
Router.route('/lists/overviewLadiesAdmins', function () {
    ...
    if (Roles.userIsInRole(Meteor.user(), 'admin', Teams.findOne({name: 'Damen'})._id)) {
        this.wait(Meteor.subscribe('usersByTeamIds', [Teams.findOne({name: 'Damen'})._id]));
